...
-- Helper: check if current user owns a session (SECURITY DEFINER bypasses RLS)
CREATE OR REPLACE FUNCTION public.owns_session(_session_id text)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
-- ═══ student_sessions ═══
DROP POLICY IF EXISTS "Anyone can read sessions" ON public.student_sessions;
DROP POLICY IF EXISTS "Anyone can upsert sessions" ON public.student_sessions;
DROP POLICY IF EXISTS "Anyone can update sessions" ON public.student_sessions;
DROP POLICY IF EXISTS "Anyone can delete sessions" ON public.student_sessions;
...
-- ═══ mastery ═══
DROP POLICY IF EXISTS "Anyone can read mastery" ON public.mastery;
DROP POLICY IF EXISTS "Anyone can insert mastery" ON public.mastery;
DROP POLICY IF EXISTS "Anyone can update mastery" ON public.mastery;
DROP POLICY IF EXISTS "Anyone can delete mastery" ON public.mastery;
...
-- ═══ teacher_messages ═══
DROP POLICY IF EXISTS "Anyone can read teacher messages" ON public.teacher_messages;
DROP POLICY IF EXISTS "Anyone can insert teacher messages" ON public.teacher_messages;
DROP POLICY IF EXISTS "Anyone can update teacher messages" ON public.teacher_messages;
DROP POLICY IF EXISTS "Anyone can delete teacher messages" ON public.teacher_messages;
