...
// ═══════════════════════════════════════════════════════════════════
// PRESENTATION GUARD — Filter out commands and meta-language
// ═══════════════════════════════════════════════════════════════════
const FORBIDDEN_PATTERNS = [
  /^\/\w+/gm,                          // Commands like /devil, /checkin
  /inventarisatie/gi,
  /diagnose/gi,
  /strategie/gi,
...
const MessageBubble: React.FC<MessageBubbleProps> = ({ message }) => {
  ...
  // Sanitize text for presentation (only for model messages)
  ...
  if (!isUser) {
    return (
      <div className="max-w-2xl mb-4">
        {/* Message Container */}
        <div className="border border-slate-700 bg-slate-800/40">
          {/* Header Bar */}
          <div className="h-8 px-3 flex items-center justify-between border-b border-slate-800 bg-slate-900/60">
            <div className="flex items-center gap-2">
              ...
              {/* Status Badges */}
              {wasRepaired && (
                <span className="ml-2 px-1.5 py-0.5 text-[9px] font-mono text-amber-300 bg-amber-500/10 border border-amber-500/40">
                  REPAIRED
                </span>
              )}
              {hasWarning && (
                <span className="ml-1 px-1.5 py-0.5 text-[9px] font-mono text-amber-300 bg-amber-500/10 border border-amber-500/40">
                  G:{Math.round((gFactor || 0) * 100)}%
                </span>
              )}
            </div>
          ...
          {/* Content */}
          <div className="px-4 py-3 text-slate-100 text-sm leading-relaxed">
            {message.isError ? (
              ...
              <ReactMarkdown
                ...
                components={{
                  ...
                  code: ({node, className, ...props}) => {
                    ...
                    if (isInline) {
                      return <code className="bg-slate-900 text-slate-300 px-1.5 py-0.5 font-mono text-xs border border-slate-800" {...props} />;
                    }
                    return <code className={className} {...props} />;
                  },

                  pre: ({node, ...props}) => (
                    <pre className="bg-slate-950 border border-slate-700 p-3 overflow-x-auto my-3 text-xs font-mono text-slate-300" {...props} />
                  ),

                  blockquote: ({node, ...props}) => (
                    <blockquote className="border-l-2 border-indigo-500/60 pl-3 italic text-slate-400 my-3 bg-slate-900/30 py-2" {...props} />
                  ),
          ...
          {/* Debug Footer (mechanical info) */}
          {message.mechanical && (
            <div className="h-7 px-3 flex items-center gap-3 border-t border-slate-800 bg-slate-900/40">
              <span className="text-[9px] font-mono text-slate-600">
                {message.mechanical.model}
              </span>
              <span className="text-slate-700">•</span>
              <span className="text-[9px] font-mono text-slate-600">
                {message.mechanical.latencyMs}ms
              </span>
              {wasRepaired && (
                <>
                  <span className="text-slate-700">•</span>
                  <span className="text-[9px] font-mono text-amber-500">
                    {message.mechanical.repairAttempts} repair{(message.mechanical?.repairAttempts || 0) > 1 ? 's' : ''}
                  </span>
                </>
              )}
            </div>
  ...
  // ═══════════════════════════════════════════════════════════════════
  // STUDENT MESSAGE (User input) — indented, lighter weight
  // ═══════════════════════════════════════════════════════════════════
  return (
    <div className="max-w-xl ml-auto mb-4">
      <div className="border border-slate-800 bg-slate-900/40">
        {/* Header Bar */}
        <div className="h-7 px-3 flex items-center justify-between border-b border-slate-800/60 bg-slate-950/40">
