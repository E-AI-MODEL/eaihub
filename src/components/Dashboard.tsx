...
interface DashboardProps {
  ...
  onEditProfile?: () => void;
  inline?: boolean; // NEW: renders as inline panel instead of overlay
}
...
const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  // ═══════════════════════════════════════════════════════════
  // INLINE MODE — renders as a panel within ResizablePanelGroup
  // ═══════════════════════════════════════════════════════════
  if (inline) {
    return (
      <div className="flex flex-col h-full bg-slate-950">
        ...
        <div className="flex-1 overflow-y-auto">
          <DashboardContent
            phase={phase}
            epistemic={epistemic}
            activeFixDetails={activeFixDetails}
            analysis={analysis}
            mechanical={mechanical}
            eaiState={eaiState}
            profile={profile}
            onEditProfile={onEditProfile}
            gFactorPercent={gFactorPercent}
        ...
        <div className="h-8 px-3 flex items-center gap-3 border-t border-slate-700 bg-slate-950 shrink-0">
          <span className="text-[9px] font-mono text-slate-600">
            {mechanical?.model || 'standby'}
          </span>
          <span className="text-slate-800">|</span>
          <span className="text-[9px] font-mono text-slate-600">
            {mechanical?.latencyMs || '—'}ms
          </span>
        </div>
      </div>
    );
  ...
  // ═══════════════════════════════════════════════════════════
  // OVERLAY MODE — legacy sliding panel (for compatibility)
  // ═══════════════════════════════════════════════════════════
  return (
    <>
      {isOpen && (
